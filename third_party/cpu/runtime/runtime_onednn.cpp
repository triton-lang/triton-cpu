...
EXPORT void *create_brgemm_ukernel(int64_t M, int64_t N, int64_t K_k,
                                   int64_t batch_size, int64_t lda, int64_t ldb, int64_t packed_ldb,
                                   int64_t ldc, int64_t dtypeA, int64_t dtypeB,
                                   int64_t dtypeC) {
  using KeyT = std::array<int64_t, 11>;
  KeyT key{M, N, K_k, batch_size, lda, ldb, packed_ldb, ldc, dtypeA, dtypeB, dtypeC};
  ...
  bool need_packing =
      brg.get_B_pack_type() == pack_type::pack32;
  if (need_packing) {
    brg = dnnl::ukernel::brgemm(M, N, K_k, batch_size, lda, packed_ldb, ldc, dnnl_dtypeA,
                              dnnl_dtypeB, dnnl_dtypeC);
    ss << "Args: M - " << M << ", N - " << N << ", K - " << K_k
          << ", batch - " << batch_size << ", lda - " << lda << ", ldb - "
          << packed_ldb << ", ldc - " << ldc << ", dtype a - " << dtypeA
...
EXPORT void *create_transform_ukernel(int64_t K, int64_t N, int64_t in_ld,
  ...
  std::stringstream ss;
  ss << "Transform args: K - " << K << ", N - " << N << ", in_ld - " << in_ld << ", out_ld - "
          << out_ld << ", in dtype - " << inDtype << ", out dtype - " << outDtype << "\n";
  std::cout << ss.str();
...
EXPORT void call_all(const void *transform_k, const void *brg_k, void *A_ptr,
  ...
  for (memory::dim i = 0; i < num_batches; i++) {
    ...
    if (need_packing) {
      ss << "Requires packing!\n";
      ss << "Orig B ptr: " << (void*)B_ptr_calc << " packed ptr: " << (void*) blocked_data << "\n";
      ss << "Repacking: b step - " << B_step_in_bytes << " block size: " << B_block_size_in_bytes << "\n";
