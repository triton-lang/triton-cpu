...
bool isOneDNNCandidate(triton::cpu::DotOp op, DotOpCandidate &candidate) {
...
Value addSubViewToCalcTPtrInfo(PatternRewriter &rewriter, Location loc,
                               Value vecVal, ValueRange indices, Value memRef) {
...
void replaceWholeLoop(DotOpCandidate &candidate,
  ...
  // auto lhsSubView = addSubViewToCalcTPtrInfo(candidate.op.getA(), lhsIndices,
  // lhsMemRef);
  ...
  // auto rhsSubView = addSubViewToCalcTPtrInfo(candidate.op.getB(), rhsIndices,
  // rhsMemRef);
...
convertCandidate(DotOpCandidate &candidate,
  ...
  if (candidate.isAccLoopCarried && candidate.canFuseLoop) {
    ...
    replaceWholeLoop(candidate, shapeInfoAnalysis, rewriter);
  ...
  auto lhsSubView = addSubViewToCalcTPtrInfo(rewriter, loc, candidate.op.getA(),
                                             candidate.lhsBuf.indices,
                                             candidate.lhsBuf.memRef);
  auto rhsSubView = addSubViewToCalcTPtrInfo(rewriter, loc, candidate.op.getB(),
                                             candidate.rhsBuf.indices,
...
struct ConvertDotToUkernels
  ...
  void runOnOperation() override {
    ...
    mod->walk([&candidates](triton::cpu::DotOp op) {
      DotOpCandidate candidate;
      if (isOneDNNCandidate(op, candidate)) {
        LLVM_DEBUG({
